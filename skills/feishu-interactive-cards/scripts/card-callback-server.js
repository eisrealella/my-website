  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      return {
        ...
        token: process.env.OPENCLAW_GATEWAY_TOKEN || config.gateway?.auth?.token || '',
...
const eventDispatcher = new lark.EventDispatcher({
  ...
  'card.action.trigger': async (data) => {
    ...
    switch (actionValue?.action) {
      ...
      case 'start_download':
        // 游戏下载处理
        const gameName = actionValue?.game_name;
        const magnetUrl = actionValue?.magnet_url;
        
        // 读取下载文件夹偏好
        const prefsPath = path.join(os.homedir(), '.openclaw', 'download_prefs.json');
        let downloadFolder = '/Users/ella/Downloads';
        try {
          if (fs.existsSync(prefsPath)) {
            const prefs = JSON.parse(fs.readFileSync(prefsPath, 'utf8'));
            downloadFolder = prefs.folder;
          }
        } catch (e) {}
        
        // 检测迅雷是否安装
        let thunderInstalled = false;
        try {
          require('child_process').execSync('test -e "/Applications/Thunder.app" && echo yes', {encoding: 'utf8'});
          thunderInstalled = true;
        } catch (e) {}
        
        // 如果没有安装，返回安装提示
        if (!thunderInstalled) {
          response = {
            toast: { type: 'warning', content: '⚠️ 未检测到迅雷' },
            card: {
              type: 'raw',
              data: {
                config: { wide_screen_mode: true },
                header: { title: { content: "⚠️ 需要安装迅雷", tag: "plain_text" }, template: "orange" },
                elements: [
                  { tag: "div", text: { content: "**📀 " + gameName + "**\n\n请先安装迅雷：\n\n```bash\nbrew install --cask thunder\n```\n\n**🧲 磁力链接：**\n`" + magnetUrl + "`", tag: "lark_md" } },
                  { tag: "action", actions: [
                    { tag: "button", text: { content: "🔄 我已安装，重试", tag: "plain_text" }, type: "primary", value: { action: "start_download", game_name: gameName, magnet_url: magnetUrl } },
                    { tag: "button", text: { content: "📋 手动下载", tag: "plain_text" }, type: "default", value: { action: "manual_download", game_name: gameName, magnet_url: magnetUrl } }
                  ]}
                ]
              }
            }
        ...
        // 已安装，返回下载确认
        response = {
          toast: { type: 'info', content: '📂 确认下载设置' },
          card: {
            type: 'raw',
            data: {
              config: { wide_screen_mode: true },
              header: { title: { content: "📁 下载设置", tag: "plain_text" }, template: "blue" },
              elements: [
                { tag: "div", text: { content: "**🎮 " + gameName + "**\n\n📂 下载至: `" + downloadFolder + "`\n\n💡 此文件夹将用于所有游戏下载", tag: "lark_md" } },
                { tag: "action", actions: [
                  { tag: "button", text: { content: "✅ 确认并下载", tag: "plain_text" }, type: "primary", value: { action: "confirm_download", game_name: gameName, magnet_url: magnetUrl, download_folder: downloadFolder } },
                  { tag: "button", text: { content: "📁 更换文件夹", tag: "plain_text" }, type: "default", value: { action: "change_folder", game_name: gameName, magnet_url: magnetUrl } }
                ]}
              ]
            }
          }
      ...
      case 'change_folder':
        response = {
          toast: { type: 'info', content: '📂 选择文件夹' },
          card: {
            type: 'raw',
            data: {
              config: { wide_screen_mode: true },
              header: { title: { content: "📁 选择下载文件夹", tag: "plain_text" }, template: "blue" },
              elements: [
                { tag: "div", text: { content: "**🎮 " + actionValue?.game_name + "**\n\n选择下载文件夹（后续游戏也将使用此文件夹）：", tag: "lark_md" } },
                { tag: "action", actions: [
                  { tag: "button", text: { content: "📥 Downloads", tag: "plain_text" }, type: "default", value: { action: "set_folder", game_name: actionValue?.game_name, magnet_url: actionValue?.magnet_url, folder: '/Users/ella/Downloads' } },
                  { tag: "button", text: { content: "🎮 Games", tag: "plain_text" }, type: "default", value: { action: "set_folder", game_name: actionValue?.game_name, magnet_url: actionValue?.magnet_url, folder: '/Users/ella/Games' } },
                  { tag: "button", text: { content: "📁 Applications", tag: "plain_text" }, type: "default", value: { action: "set_folder", game_name: actionValue?.game_name, magnet_url: actionValue?.magnet_url, folder: '/Users/ella/Applications' } }
                ]}
              ]
            }
          }
      ...
      case 'set_folder':
        const setGame = actionValue?.game_name;
        const setMagnet = actionValue?.magnet_url;
        const setFolder = actionValue?.folder;
        
        // 保存用户偏好
        try {
          fs.writeFileSync(prefsPath, JSON.stringify({ folder: setFolder, lastUpdated: new Date().toISOString() }, null, 2));
        } catch (e) {}
        
        // 执行下载（优先 Peekaboo bridge）
        const scriptPath = path.join(__dirname, 'thunder-download.sh');
        let setDownloadOk = false;
        let setDownloadError = '';
        try {
          require('child_process').execSync(`bash "${scriptPath}" "${setMagnet}"`, {
            encoding: 'utf8',
            timeout: 30000,
            stdio: 'pipe'
          });
          ...
          setDownloadError = (e && (e.stderr || e.message)) ? String(e.stderr || e.message).slice(0, 300) : '执行下载脚本失败';
        }

        if (setDownloadOk) {
          response = {
            toast: { type: 'success', content: '✅ 下载已开始' },
            card: {
              type: 'raw',
              data: {
                config: { wide_screen_mode: true },
                header: { title: { content: "✅ 下载已开始", tag: "plain_text" }, template: "green" },
                elements: [
                  { tag: "div", text: { content: "**🎮 " + setGame + "**\n\n✅ 磁力链接已发送至下载软件\n\n📂 下载至: `" + setFolder + "`\n\n💡 已记住此文件夹", tag: "lark_md" } },
                  { tag: "hr" },
                  { tag: "div", text: { content: "ℹ️ 此文件夹将用于后续所有游戏下载", tag: "lark_md" } },
                  { tag: "action", actions: [
                    { tag: "button", text: { content: "🔙 返回", tag: "plain_text" }, type: "default", value: { action: "back" } }
                  ]}
                ]
              }
            }
          ...
          response = {
            toast: { type: 'warning', content: '⚠️ 自动下载未完成，请重试或手动下载' },
            card: {
              type: 'raw',
              data: {
                config: { wide_screen_mode: true },
                header: { title: { content: "⚠️ 自动下载失败", tag: "plain_text" }, template: "orange" },
                elements: [
                  { tag: "div", text: { content: "**🎮 " + setGame + "**\n\n自动点击下载按钮失败。你可以重试，或选择手动下载。\n\n错误信息：\n`" + setDownloadError + "`", tag: "lark_md" } },
                  { tag: "action", actions: [
                    { tag: "button", text: { content: "🔄 重试下载", tag: "plain_text" }, type: "primary", value: { action: "confirm_download", game_name: setGame, magnet_url: setMagnet, download_folder: setFolder } },
                    { tag: "button", text: { content: "📋 手动下载", tag: "plain_text" }, type: "default", value: { action: "manual_download", game_name: setGame, magnet_url: setMagnet } }
                  ]}
                ]
              }
            }
      ...
      case 'confirm_download':
        const cfGame = actionValue?.game_name;
        const cfMagnet = actionValue?.magnet_url;
        const cfFolder = actionValue?.download_folder;
        
        // 执行下载（优先 Peekaboo bridge）
        const scriptPath2 = path.join(__dirname, 'thunder-download.sh');
        let confirmDownloadOk = false;
        let confirmDownloadError = '';
        try {
          require('child_process').execSync(`bash "${scriptPath2}" "${cfMagnet}"`, {
            encoding: 'utf8',
            timeout: 30000,
            stdio: 'pipe'
          });
          confirmDownloadOk = true;
        } catch (e) {
          confirmDownloadError = (e && (e.stderr || e.message)) ? String(e.stderr || e.message).slice(0, 300) : '执行下载脚本失败';
        }

        if (confirmDownloadOk) {
          response = {
            toast: { type: 'success', content: '✅ 下载已开始' },
            card: {
              type: 'raw',
              data: {
                ...
                header: { title: { content: "✅ 下载已开始", tag: "plain_text" }, template: "green" },
                elements: [
                  { tag: "div", text: { content: "**🎮 " + cfGame + "**\n\n✅ 磁力链接已发送至迅雷\n\n📂 下载至: `" + cfFolder + "`", tag: "lark_md" } },
                  { tag: "action", actions: [
                    { tag: "button", text: { content: "🔙 返回", tag: "plain_text" }, type: "default", value: { action: "back" } }
                  ]}
                ]
          ...
          response = {
            ...
            card: {
              ...
              data: {
                ...
                header: { title: { content: "⚠️ 自动下载失败", tag: "plain_text" }, template: "orange" },
                elements: [
                  { tag: "div", text: { content: "**🎮 " + cfGame + "**\n\n自动点击下载按钮失败。你可以重试，或选择手动下载。\n\n错误信息：\n`" + confirmDownloadError + "`", tag: "lark_md" } },
                  { tag: "action", actions: [
                    { tag: "button", text: { content: "🔄 重试下载", tag: "plain_text" }, type: "primary", value: { action: "confirm_download", game_name: cfGame, magnet_url: cfMagnet, download_folder: cfFolder } },
                    { tag: "button", text: { content: "📋 手动下载", tag: "plain_text" }, type: "default", value: { action: "manual_download", game_name: cfGame, magnet_url: cfMagnet } }
                  ]}
                ]
              }
            }
      ...
      case 'manual_download':
        response = {
          toast: { type: 'info', content: 'ℹ️ 手动下载' },
          card: {
            type: 'raw',
            data: {
              config: { wide_screen_mode: true },
              header: { title: { content: "📋 手动下载", tag: "plain_text" }, template: "blue" },
              elements: [
                { tag: "div", text: { content: "**🎮 " + actionValue?.game_name + "**\n\n**🧲 磁力链接：**\n\n`" + actionValue?.magnet_url + "`\n\n使用方法：\n1. 复制上方链接\n2. 打开你的下载软件\n3. 粘贴链接开始下载", tag: "lark_md" } },
                { tag: "action", actions: [
                  { tag: "button", text: { content: "🔙 返回", tag: "plain_text" }, type: "default", value: { action: "back" } }
                ]}
              ]
